...
@pytest.mark.asyncio
async def test_board_created_successfully(board_service, mock_uow, sample_board):
...
@pytest.mark.asyncio
async def test_board_retrieved_by_id(board_service, mock_uow, sample_board):
...
@pytest.mark.asyncio
async def test_board_not_found_raises_exception(board_service, mock_uow):
    board_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_published_from_draft_board(board_service, mock_uow, sample_board):
    board_id = sample_board.id
...
@pytest.mark.asyncio
async def test_closed_board_cannot_be_reopened(board_service, mock_uow, sample_board):
...
@pytest.mark.asyncio
async def test_changing_to_draft_removes_all_votes(board_service, mock_uow, sample_board):
